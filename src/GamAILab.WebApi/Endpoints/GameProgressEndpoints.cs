...
public static class GameProgressEndpoints
{
    public static IEndpointRouteBuilder MapGameProgressEndpoints(this IEndpointRouteBuilder app)
    {
        ...
        group.MapGet("/custom-data", async Task<Ok<List<CustomData>>> (IGameService gameService, ClaimsPrincipal user) =>
            ...
            .Produces<List<CustomData>>(StatusCodes.Status200OK);
        
        
        group.MapGet("/custom-data/{key}", async Task<Results<Ok<CustomData>, NotFound, BadRequest<string>>> (
                string key, IGameService gameService, ClaimsPrincipal user) =>
            {
                var userId = GetUserId(user);
        ...
        group.MapGet("/objectives", async Task<Ok<List<GameObjective>>> (
                IGameService gameService, ClaimsPrincipal user) =>
            {
                var userId = GetUserId(user);
            ...
            .Produces<List<GameObjective>>(StatusCodes.Status200OK);

        group.MapGet("/objectives/{objectiveId}", async Task<Results<Ok<GameObjective>, NotFound, BadRequest<string>>> (
                string objectiveId, IGameService gameService, ClaimsPrincipal user) =>
            {
                var userId = GetUserId(user);
